# Update MBA Project Report with Enhanced Metrics
# Integrates all new FinOps metrics, forecast accuracy, and comprehensive data
# For: Predictive AWS Cost Optimization - Prophet Forecasting
...
def generate_updated_report():
    """
    Generate updated MBA project report text with enhanced metrics
    """
    
    print("\n" + "="*80)
    print("📝 GENERATING UPDATED MBA PROJECT REPORT")
    print("="*80 + "\n")
    
    # Load the enhanced data
    df = pd.read_csv('aws_cost_data_enhanced_with_finops.csv')
    ...
    potential_savings = df['potential_rightsizing_savings_inr'].sum()
...
2. Real-time waste detection and rightsizing recommendations (₹{potential_savings:,.0f}/month potential)
...
Combined impact: Annual savings potential of ₹{(potential_savings * 12):,.0f} from rightsizing, plus waste reduction from {waste_rate:.1f}% toward industry best practice of <15%, resulting in total estimated annual savings exceeding ₹3,50,000 for organizations at this scale.
...
2. Governance & Compliance:
   ...
   - Current compliance: {tag_compliance:.1f}%
...
D. FinOps KPIs Tracked:
   • Waste Rate: {waste_rate:.2f}% (Target: <15%)
   • Tag Compliance: {tag_compliance:.1f}% (Target: ≥90%)
   • RI/SP Coverage: 22.48% (Target: 60-80%)
   • RI/SP Utilization: 90.79% (Target: ≥90%) ✅
   • Avg CPU Utilization: {avg_utilization:.1f}% (Target: ≥70%) ✅
...
₹{total_idle:,.0f} of the total ₹{total_cost:,.0f} ({waste_rate:.2f}%) is going to idle/underutilized resources:
- Above industry best practice target of <15%
- Rightsizing opportunities: ₹{potential_savings:,.0f}/month
...
- Average CPU Utilization: {avg_utilization:.1f}% (Target: ≥70%) ✅
...
2. **Address waste and rightsizing**:
   - Target the {waste_rate:.2f}% waste rate toward <15% industry best practice
   - Implement ₹{potential_savings:,.0f}/month in identified savings
...
5. **Improve tag compliance**:
   - Current: {tag_compliance:.1f}% → Target: ≥90%
...
Immediate (Month 1-3):
• Waste reduction: ₹{potential_savings:,.0f}/month
...
The downward trend we found? That tells us optimization efforts are working. The seasonal pattern? Now you know exactly when to scale up (Q4) and when to scale down (Q2). The waste identification? ₹{potential_savings:,.0f}/month in quick wins sitting right there.
...
COST OPTIMIZATION         Total Spend                      ₹{total_cost:,.0f}         -             -
                          Waste Rate                       {waste_rate:.2f}%          <15%          ⚠️  Reduce
                          Potential Savings                ₹{potential_savings:,.0f}/mo        -             ✅ Identified

GOVERNANCE                Tag Compliance                   {tag_compliance:.1f}%           ≥90%          ⚠️  Improve
                          RI/SP Coverage                   22.48%           60-80%        ⚠️  Increase
                          RI/SP Utilization                90.79%           ≥90%          ✅ Optimal

UTILIZATION               Avg CPU Utilization              {avg_utilization:.1f}%           ≥70%          ✅ Healthy
...
"""
    ...
    # Save to file
    output_file = 'MBA_Project_Report_UPDATED_WITH_METRICS.txt'
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(report)
    
    print(f"✅ Updated report saved to '{output_file}'")
    print(f"\n📊 Report includes:")
    print(f"   • Enhanced Abstract with all new metrics")
    print(f"   • Comprehensive Methodology section")
    print(f"   • Detailed Findings with 10 key insights")
    print(f"   • Actionable Recommendations (immediate/short/long-term)")
    print(f"   • Enhanced Conclusion")
    print(f"   • Key Metrics Summary Table")
